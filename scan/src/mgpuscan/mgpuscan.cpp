...
const int ScanValuesPerBlocks = ScanValuesPerThread * NumScanThreads;
...
void SetBlockRanges(std::vector<int2>& ranges, int numBlocks, int count, 
	bool segScan) {
...
scanStatus_t SCANAPI scanArray(scanEngine_t engine, CUdeviceptr data, int count,
	...
	SetBlockRanges(ranges, engine->numBlocks, count, false);
